...
$sql = "SELECT staff_id, full_name, department, campus,date_on_present_grade, highest_qualification, dob, gender FROM staff WHERE designation = 'Senior Lecturer' AND gender = 'Female'";
...
<head>
    ...
    <title>Female Senior Lecturers List</title>
...
<body>
    ...
    <div class="uhas-header">
        <h1>Female Senior Lecturers List</h1>
    </div>

    <div class="container my-4">
        <a href="reports.php" class="btn btn-uhas mb-3">Back</a>
       <table id="seniorLecturersTable" class="table table-striped table-bordered">
            <thead>
                <tr>
                    <th>S/NO</th>
                    <th>Staff ID</th>
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            // Calculate age if 'date_of_birth' column is present
            $age = "N/A"; // Default value if date_of_birth is not set or invalid
            if (!empty($row['dob'])) {
    ...
    ?>
        ...
        <p class="text-end"><strong>Total Female Senior Lecturers:</strong> <?= $count; ?></p>
    </div>
    ...
    <script>
        $(document).ready(function() {
            $('#seniorLecturersTable').DataTable({
