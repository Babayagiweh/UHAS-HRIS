...
// Fetch data for "Casual leave" only with optional search
$sql = "SELECT * FROM `leave` WHERE leave_type = 'Casual leave' AND (full_name LIKE '%$search%' OR staff_id LIKE '%$search%') LIMIT $start, $limit";
$result = $conn->query($sql);

// Total number of records for pagination
$total_sql = "SELECT COUNT(*) AS total FROM `leave` WHERE leave_type = 'Casual leave' AND (full_name LIKE '%$search%' OR staff_id LIKE '%$search%')";
...
<body>
    ...
    <header class="uhas-header">
        ...
        <p>Casual Leave Report</p>
    </header>

    <div class="container">
        <h2 class="mb-4 text-center">Staff Casual Leave Data</h2>
        ...
        <nav>
            <ul class="pagination justify-content-center">
                ...
                for ($i = 1; $i <= $total_pages; $i++) {
                    ...
                    echo "<li class='page-item $active'><a class='page-link' href='casual_leave_reports.php?page=$i&search_term=$search'>$i</a></li>";
                }
                ?>
            </ul>
        </nav>
    ...
    <script>
        function exportToExcel() {
            ...
            var wb = XLSX.utils.table_to_book(table, { sheet: "Casual Leave Data" });
            XLSX.writeFile(wb, "Casual_Leave_Report.xlsx");
        }

        function exportToPDF() {
            var doc = new jsPDF();
            doc.autoTable({ html: 'table' });
            doc.save('Casual_Leave_Report.pdf');
        }
    </script>
