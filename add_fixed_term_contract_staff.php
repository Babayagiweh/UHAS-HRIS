...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_individual'])) {
    ...
    // Insert into fixed_term_contracts table
    $sql = "INSERT INTO fixed_term_contracts (staff_id, full_name, dob, designation, present_appointment, 
            department, highest_qualifications, service_date, contract_start_date, contract_end_date, 
            campus, email, phone, files) 
            VALUES ('$staff_id', '$full_name', '$dob', '$designation', '$present_appointment', 
            '$department', '$highest_qualifications', '$service_date', '$contract_start_date', 
            '$contract_end_date', '$campus', '$email', '$phone', '$files')";

    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('Fixed term contract staff information added successfully!');</script>";
    } else {
        ...
        if (($handle = fopen($file, "r")) !== FALSE) {
            ...
            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                if ($row++ > 0) { // Skip the header row
                    ...
                    // Insert each row of data into the fixed_term_contracts table
                    $sql = "INSERT INTO fixed_term_contracts (staff_id, full_name, dob, designation, present_appointment, 
                            department, highest_qualifications, service_date, contract_start_date, contract_end_date, 
                            campus, email, phone, files) 
                            VALUES ('$staff_id', '$full_name', '$dob', '$designation', '$present_appointment', 
            ...
            fclose($handle);
            echo "<script>alert('Bulk fixed term contract staff data uploaded successfully!');</script>";
        }
    } else {
...
if (isset($_GET['export_template'])) {
    ...
    header('Content-Disposition: attachment; filename="fixed_term_contracts_template.csv"');
...
<head>
    ...
    <title>Add Fixed Term Contract Staff</title>
...
<header class="uhas-header">
    ...
    <p>Add Fixed Term Contract Staff Information</p>
</header>

<div class="container">
    <a href="fixed_term_contracts_mgt.php" class="btn btn-uhas mb-3">Back</a>
    <h2 class="mb-4 text-center">Fixed Term Contract Staff Form</h2>
    <form method="POST" enctype="multipart/form-data">
        ...
        <button type="submit" class="btn btn-uhas mt-3" name="submit_individual">Add Fixed Term Contract Staff</button>
    </form>

    <div class="mt-5">
        <h3>Bulk Upload Fixed Term Contract Staff Data</h3>
        <form method="POST" enctype="multipart/form-data">
            <div class="form-group">
